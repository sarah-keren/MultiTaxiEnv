...
The environment is passed a function via the config.py file, and uses this function to calculate the rewards of each
turn.
The different reward functions are implemented in this file.
"""
from Domains.Taxis.multitaxienv.config import TAXI_ENVIRONMENT_REWARDS
from Domains.Taxis.multitaxienv.taxi_utils import basic_utils


def partial_closest_path_reward(state: list, basic_reward_str: str, taxi_index: int = None, reward_const: int = 15) -> int:
    """
    ...
    return reward_const * (basic_utils.passenger_destination_l1_distance(state, passenger_index,
