...
import itertools
import random
from .config import multitaxi_rewards_w_idle
...
class MultiTaxiEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        for taxi, action in enumerate(actions):
            ...
            reward = multitaxi_rewards_w_idle['step']  # default reward when there is no pickup/dropoff
            done = False

            if not is_engine_on:
                if action == 6:  # standby while engine is off
                    reward = multitaxi_rewards_w_idle['standby_engine_off']
                elif action == 7:  # turn engine on
                    reward = multitaxi_rewards_w_idle['turn_engine_on']
    ...
    def render(self, mode='human', diag=True):
        ...
        if self.lastaction is not None:
            moves = ["South", "North", "East", "West", "Pickup", "Dropoff",
                     "Standby Engine Off", "Turn Engine On", "Turn Engine Off", "Standby Engine On",
                     "Refuel"]
